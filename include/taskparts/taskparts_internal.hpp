...
static void afprintf(FILE *stream, const char *fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
  {
...
static void aprintf(const char *fmt, ...) {
  {
    std::unique_lock<std::mutex> lk(print_mutex);
...
static constexpr int cache_line_szb = 64;

#if defined(TASKPARTS_X64)
static constexpr int cpu_context_szb = 8 * 8;
#elif defined(TASKPARTS_ARM64)
static constexpr int cpu_context_szb = 21 * 8;
#endif
...
public:
  alignas(cache_line_szb) std::atomic<int> incounter;
  alignas(cache_line_szb) native_fork_join_vertex *outset;
  alignas(cache_line_szb) native_continuation continuation;
  ...
  auto decrement(const Schedule &schedule) -> void {
    ...
    if (alternative == clone_fast) {
      incounter.store(incounter.load(std::memory_order_relaxed) - 1,
...
template <typename F> auto launch(F &&f) -> void {
...
public:
  ...
  virtual auto self() -> vertex * = 0;
  virtual auto initialize_fork(vertex *parent, vertex *child1,
                               vertex *child2) -> void = 0;
  virtual auto worker_continuation() -> continuation & = 0;
  virtual auto pop() -> vertex * = 0;
  virtual auto schedule(vertex *v) -> void = 0;
};

extern auto my_scheduler() -> native_fork_join_scheduler_interface *;

extern "C" void *context_save(char *);

template <typename F1, typename F2>
__attribute__((returns_twice)) auto fork2join(F1 &&f1, F2 &&f2) -> void {
  if (in_scheduler) {
    auto f11 = [] {};
    ...
    if (context_save(&(v0.continuation.gprs[0]))) {
      {
        assert(s.self() == &v0);
        assert(v0.continuation.action == continuation_continue);
      }
    ...
    swap(v1.continuation, s.worker_continuation());
    { assert(s.self() == &v1); }
    ...
    {
      assert(vb == &v2);
      assert(v2.incounter.load() == 0);
      assert(v0.incounter.load() == 2);
    }
    ...
    {
      assert(s.self() == &v0);
      assert(v0.incounter.load() == 0);
    }
    v0.continuation.action = continuation_finish;
    // exit point of parent task if v2 was not stolen
  } else {
    { assert(get_my_id() == 0); } // need to launch a new scheduler instance
    in_scheduler = true;
...
public:
  alignas(cache_line_szb) std::atomic<uint64_t> incounter;
  alignas(cache_line_szb) void *outset;
...
template <typename Edge_endpoints = fork_join_edges,
          typename Continuation = continuation>
class vertex_family {
public:
  alignas(cache_line_szb) Edge_endpoints edges;
  alignas(cache_line_szb) Continuation continuation;

  virtual ~vertex_family() {}
  virtual auto run() -> void = 0;
  virtual auto deallocate() -> void = 0;
};
...
auto parallel_notify(Vertex_handle v) -> void {
  v->edges.parallel_notify(
      [](void *_v) { decrement<Scheduler>((Vertex_handle)_v); });
}
...
template <typename F> class dag_calculus_vertex : public vertex {
public:
  F f;
...
template <typename F>
auto create_vertex(const F &f, continuation_types continuation_type =
