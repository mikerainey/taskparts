...
template <typename F> auto test_variable_workload(const F &f) {
  auto max_items = 1 << 10;
  size_t i = 0;
  uint64_t r = 1234;
  fork2join(
      [&] {
        while (f()) {
          ...
          r = reduce([&](uint64_t i) { return hash(i); },
                     [&](uint64_t r1, uint64_t r2) { return hash(r1 | r2); }, r,
                     0, n);
        }
      },
      [] {});
...
auto test_variable_workload() {
  auto secs = 4.0;
  auto s = now();
  test_variable_workload([&] { return since(s) < secs; });
}
...
template <int nb_labels, typename Frame> class cfg_definition {
public:
  // basic blocks
  ...
  cfg_definition()
      : is_exit_block([](trampoline_block_label l) {
          return l == nb_labels; // defaultly, the last block is the one and
                                 // only exit block
        }) {
...
template <int nb_labels, typename Frame> class cfg_vertex : public vertex {
public:
  static cfg_definition<nb_labels, Frame> cfg;

  alignas(cache_line_szb) Frame frame;

  cfg_vertex() : vertex() {
    continuation.continuation_type = continuation_trampoline;
    continuation.u.t.next =
        0; // the first block is the one and only entrance block
  }
  auto run() -> void {
  ...
  auto deallocate() -> void { delete this; }
};
...
// TODOs:
//   - refactor serial_scheduler so that it can be instantiated into DFS, BFS,
//   pseudo-random schedules
//   - can native fork join be implemented using C++ coroutines and on demand
//   spawning of threads?
//   - fix thunk issue in ucontext by moving the thunk into the ucontext class
//   - fix space leak in the stack used by ucontext
//   - find a better way of passing continuation-action signals in ucontext,
//   which currently has to signal the termination of a task by performing
//   updates at exit points
//   - randomized dag generators
//   - add proper implementation for DAG calculus outset

template <typename Vertex_handle = vertex *> class dag_calculus_scheduler {
public:
  virtual auto schedule(Vertex_handle t) -> void = 0;
  virtual auto create_task(thunk f,
                           continuation_types continuation_type =
                               continuation_ucontext) -> Vertex_handle = 0;
  virtual auto release(Vertex_handle v) -> void = 0;
  virtual auto new_edge(Vertex_handle v, Vertex_handle u) -> void = 0;
  virtual auto yield() -> void = 0;
  virtual auto self() -> Vertex_handle = 0;
  virtual auto launch(
      Vertex_handle root_task,
      continuation_types continuation_type = continuation_ucontext) -> void = 0;
};

template <typename Vertex_handle>
using dag_calculus_scheduler_stack =
    std::deque<dag_calculus_scheduler<Vertex_handle> *>;
...
public:
  ...
  public:
    alignas(cache_line_szb) thunk body;
  ...
  auto loop() -> void {
    while (!ready.empty()) {
      ...
      if (get_action(current->continuation) == continuation_initialize) {
        new_continuation(current->continuation, [] {
          ((serial_dag_calculus_scheduler *)my_scheduler())->enter();
        });
      }
  ...
  auto create_task(thunk f, continuation_types continuation_type =
  ...
  auto self() -> vertex * { return current; }
  auto
  launch(vertex *root_task,
  ...
  static auto Schedule(vertex *v) -> void { my_scheduler()->schedule(v); }
};

auto launch_rec(dag_calculus_scheduler_stack<vertex *> &stack, size_t position,
                dag_calculus_scheduler<vertex *> *target, thunk f) -> void {
  ...
  if (s == target) {
    ...
    s->launch(
        s->create_task([&] { launch_rec(stack, position + 1, target, f); }));
  }
}
...
auto launch(thunk f, continuation_types continuation_type =
                         continuation_ucontext) -> void {
...
auto release(vertex *v) -> void { my_scheduler()->release(v); }
auto self() -> vertex *;
auto new_edge(vertex *v, vertex *u = self()) -> void {
  my_scheduler()->new_edge(v, u);
}
auto yield() -> void { my_scheduler()->yield(); }
auto self() -> vertex * { return my_scheduler()->self(); }
auto launch(vertex *root_task, continuation_types continuation_type =
                                   continuation_ucontext) -> void {
...
auto fib_minimal(uint64_t n, uint64_t *dst) -> void {
  ...
  vertex *vs[2];
  for (int i = 1; i >= 0; i--) {
    vs[i] = create_task([=] { fib_minimal(n - (i + 1), &dst2[i]); },
                        continuation_minimal);
  }
  for (auto v : vs) {
    new_edge(v, vj);
...
using fib_trampoline = enum fib_trampoline_enum : int {
  fib_entry = 0,
  fib_combine,
  fib_exit,
  fib_nb_blocks = fib_exit
};
using fib_frame = struct fib_struct {
  uint64_t n;
  uint64_t *dst;
  uint64_t *dst2;
};
...
auto define_fib_cfg() -> void {
  ...
  blocks[fib_entry] =
      std::function<fib_trampoline(fib_frame &)>([](fib_frame &f) {
        if (f.n < 2) {
          *f.dst = f.n;
          return fib_exit;
        }
...
auto fib_ucontext(uint64_t n) -> uint64_t {
  ...
  vertex *vs[2];
  for (int i = 1; i >= 0; i--) {
    vs[i] = create_task([=, &dst2] { dst2[i] = fib_ucontext(n - (i + 1)); });
  }
  for (auto v : vs) {
    new_edge(v);
  }
...
auto test_nested_scheduler() -> void {
  ...
  auto root_task1 = s1.create_task(
      [&] {
        ...
        i++;
        auto root_task2 = s2.create_task([&] { i++; }, continuation_minimal);
...
auto test_fib_dag_calculus() -> void {
  uint64_t dst = 123;
  launch_dag_calculus(dag_calculus_scheduler::create_vertex(
      new dag_calculus_vertex([&] { fib_dag_calculus(10, &dst); }),
      continuation_minimal));
  printf("dst=%lu\n", dst);
}
...
public:
  ...
  auto add(thunk f) -> void { tests.push_back(f); }
  auto run() -> void {
...
int main() { /*
  ...
  tests.add([] { // basic dag calculus
    ...
    schedulers.push_back(&s);
    auto root_task =
        s.create_task([&] { fib_minimal(n, &dst); }, continuation_minimal);
