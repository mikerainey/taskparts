...
auto sum_rec(node *n) -> int {
  if (n == nullptr) {
    ...
    int s0, s1;
    parlay::par_do([&] { s0 = sum_rec(n->left); },
                   [&] { s1 = sum_rec(n->right); });
...
template <typename Seq = std::deque<vkont>> auto sum_iterative(node *n) -> int {
  Seq k({vkont(K3)});
...
public:
  ...
  vhbkont(kont_tag tag, node *n, tpalrts_prml_node *prev) // K1
      : tag(tag) {
    u.k1.n = n;
    u.k1.prml_node.prev = prev;
    u.k1.prml_node.next = nullptr;
  }
...
auto tpalrts_prmlist_push_back(tpalrts_prml prml,
                               tpalrts_prml_node *t) -> tpalrts_prml {
...
auto sum_heartbeat_loop(node *n, std::deque<vhbkont> &k,
                        tpalrts_prml prml) -> void;

template <typename F> auto create_task(const F &f) -> task * {
...
auto gen_perfect_tree(int height) -> parlay::sequence<node> {
  ...
  auto has_children = [&](int nd) -> bool { return child(nd, 2) < n; };
  auto nodes = parlay::to_sequence(
      parlay::delayed_tabulate(n, [&](size_t i) { return node(1); }));
...
std::function<node *(std::deque<node> &)> default_update_leaf =
...
auto gen_random_path_update(node *root, size_t &rng, std::deque<node> &path,
                            std::function<node *(std::deque<node> &)>
                                update_leaf = default_update_leaf) -> void {
...
auto gen_random_updates(node *root, size_t n,
                        std::deque<std::deque<node>> &updates,
                        std::function<node *(std::deque<node> &)> update_leaf =
                            default_update_leaf) -> void {
  node *_root = root;
  size_t rng = 1243;
...
auto gen_linear_path(size_t length, std::deque<node> &path,
                     size_t branch = 0) -> node * {
  if (length == 0) {
    return nullptr;
  }
  path.push_back(node(1));
...
int main() {
  ...
  auto update_path_length =
      std::max(1, cmdline::parse_or_default_int("update_path_length", 1));
  auto branch = cmdline::parse_or_default_int("branch", 0) % 2;
  std::deque<std::deque<node>> updates;
  auto nodes = gen_perfect_tree(height);
  gen_random_updates(&nodes[0], nb_updates, updates,
                     [&](std::deque<node> &path) -> node * {
  ...
  d.add("recursive", [&] { taskparts::benchmark([&] { s = sum_rec(root); }); });
  d.add("iterative",
        [&] { taskparts::benchmark([&] { s = sum_iterative(root); }); });
  d.add("heartbeat",
        [&] { taskparts::benchmark([&] { s = sum_heartbeat(root); }); });
  d.add("opencilk",
        [&] { taskparts::benchmark_opencilk([&] { s = sum_rec(root); }); });
