...
void report_correct(row result, row labels) {
  ...
  size_t num_correct = parlay::reduce(parlay::tabulate(
      n, [&](size_t i) { return (result[i] == labels[i]) ? 1 : 0; }));
  float percent_correct = (100.0 * num_correct) / n;
  std::cout << num_correct << " correct out of " << n << ", " << percent_correct
            << " percent" << std::endl;
}
...
auto read_data(std::string filename) {
  ...
  auto types = parlay::map(parlay::tokens(head, is_item),
...
auto rows_to_features(sequence<char> types, rows const &A) {
  ...
  auto get_feature = [&](size_t io) {
    ...
    auto vals =
        parlay::tabulate(num_rows, [&](size_t j) -> value { return A[j][i]; });
...
auto read_features(std::string filename) {
  ...
  auto test_labels =
      parlay::map(test_rows, [&](auto &row) { return row[row.size() - 1]; });
  return std::tuple(rows_to_features(types, train_rows), test_rows,
                    test_labels);
}
