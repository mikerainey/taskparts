...
int main(int argc, char *argv[]) {
  ...
  if (argc != 2)
    ...
    auto order =
        stable_sort(verts, [&](vertex u, vertex v) { return R[u] < R[v]; });

    utils::print_graph_stats(G);
    result result;
    taskparts::benchmark([&] { result = create_le_list(G, GT, order); });

    long total = reduce(map(result, parlay::size_of()));
    std::cout << "Average LE-list size: " << ((double)total / (double)n)
              << std::endl;
  }
}
