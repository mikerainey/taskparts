  ...
  if (argc != 2)
    ...
    if (n == 0) { // read triangles from a file
      ...
      auto minf = [](point3d a, point3d b) {
        return point3d{std::min(a[0], b[0]), std::min(a[1], b[1]),
                       std::min(a[2], b[2])};
      };
      auto maxf = [](point3d a, point3d b) {
        return point3d{std::max(a[0], b[0]), std::max(a[1], b[1]),
                       std::max(a[2], b[2])};
      };
      minp = parlay::reduce(points, parlay::binary_op(minf, points[0]));
      maxp = parlay::reduce(points, parlay::binary_op(maxf, points[0]));
      auto corners =
          parlay::map(words.cut(2 + 3 * num_points, words.size()),
                      [&](auto s) { return parlay::chars_to_long(s); });
      triangles = parlay::tabulate(num_triangles, [&](long i) {
        return triangle{points[corners[3 * i] - 1],
                        points[corners[3 * i + 1] - 1],
    ...
    auto rays = parlay::tabulate(n, [&](long i) {
      ...
      auto d = maxp - minp;
      return ray{
          point3d{minp[0] + d[0] * dis(r), minp[1] + d[1] * dis(r), minp[2]},
          point3d{minp[0] + d[0] * dis(r), minp[1] + d[1] * dis(r), maxp[2]}};
    });

    parlay::sequence<index_t> r;
    taskparts::benchmark([&] { r = ray_cast(triangles, rays); });

    long cnt = parlay::reduce(
        parlay::map(r, [](index_t i) { return (i < 0) ? 0 : 1; }));
    std::cout << cnt << " rays intersect a triangle out of " << rays.size()
              << std::endl;
  }
}
