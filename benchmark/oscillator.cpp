...
class rational {
public:
  rational(int64_t numerator = 1, int64_t denominator = 1)
  ...
  rational operator+(const rational &rhs) const {
    ...
    int64_t numerator_sum = (num * (common_denominator / den)) +
                            (rhs.num * (common_denominator / rhs.den));
  ...
  int64_t ceiling() const {
    // For positive numbers, we can use std::ceil after converting to a floating
    // point. For negative numbers, the ceiling is just the division of num by
    // den.
    if (num > 0) {
  ...
  // Outputs the rational number to a stream in the form
...
auto reduce(const F &f, const R &r, V z, size_t lo, size_t hi,
            size_t grain = 2) -> V {
  if (lo == hi) {
    return z;
  }
...
inline uint64_t _hash(uint64_t u) {
...
auto oscillate(rational max_n, rational step, size_t grain) -> void {
  ...
  while (since(s) < secs) {
    r = reduce([&](uint64_t i) { return _hash(i); },
               [&](uint64_t r1, uint64_t r2) { return _hash(r1 | r2); }, r, 0,
               n.ceiling(), grain);
  ...
  std::cout << "nb_apps=" << nb_apps << " rounds=" << rounds << " r=" << r
            << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 4) {
    ...
    uint64_t grain = 2;
    try {
      max_n = rational::fromString(argv[1]);
    } catch (...) {
      printf("%s\n", usage);
    }
    try {
      step = rational::fromString(argv[2]);
    } catch (...) {
      printf("%s\n", usage);
    }
    try {
      grain = std::stol(argv[3]);
    } catch (...) {
      printf("%s\n", usage);
    }
    parlay::par_do(
        [&] { // to ensure that the taskparts scheduler is running
          taskparts::benchmark([&] { oscillate(max_n, step, grain); });
        },
        [&] {});
  }
  return 0;
